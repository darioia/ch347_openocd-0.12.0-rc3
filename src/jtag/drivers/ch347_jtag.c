...
#ifndef _WIN32
#include <jtag/drivers/ch347dll.h>
#endif
...
#define __stdcall
typedef /*int*/ HANDLE (__stdcall *pCH347OpenDevice)(unsigned long iIndex);
typedef /*void*/ BOOL (__stdcall *pCH347CloseDevice)(unsigned long iIndex);
typedef /*unsigned long*/ BOOL (__stdcall *pCH347SetTimeout)(unsigned long iIndex,        // 指定设备序号
                                                   unsigned long iWriteTimeout, // 指定USB写出数据块的超时时间,以毫秒mS为单位,0xFFFFFFFF指定不超时(默认值)
                                                   unsigned long iReadTimeout); // 指定USB读取数据块的超时时间,以毫秒mS为单位,0xFFFFFFFF指定不超时(默认值)
typedef /*unsigned long*/ BOOL (__stdcall *pCH347WriteData)(unsigned long iIndex,         // 指定设备序号
                                                  void *oBuffer,                // 指向一个足够大的缓冲区,用于保存描述符
                                                  unsigned long *ioLength);     // 指向长度单元,输入时为准备读取的长度,返回后为实际读取的长度
typedef /*unsigned long*/ BOOL (__stdcall *pCH347ReadData)(unsigned long iIndex,          // 指定设备序号
                                                 void *oBuffer,                 // 指向一个足够大的缓冲区,用于保存描述符
                                                 unsigned long *ioLength);      // 指向长度单元,输入时为准备读取的长度,返回后为实际读取的长度
typedef /*unsigned long*/ BOOL (__stdcall *pCH347Jtag_INIT)(unsigned long iIndex,         // 指定设备序号
                                                  unsigned char iClockRate);    // 指向长度单元,输入时为准备读取的长度,返回后为实际读取的长度
BOOL ugOpen;
unsigned long ugIndex;
pCH347OpenDevice upOpenDev;
 ...
 *  @return 		  写成功返回1，失败返回0
 */
static int CH347_Write(/*void*/ unsigned char *oBuffer, unsigned long *ioLength)
{
    int ret = -1;
    ...
    while (1) {
        ...
        LOG_DEBUG_IO("(size=%ld, buf=[%s]) -> %" PRIu32, wlength, HexToString((uint8_t *)oBuffer, wlength), (int)wlength);
        WI += wlength;
        if (WI >= *ioLength)
            break;
 ...
 * @return 			读成功返回1，失败返回0
 */
static int CH347_Read(void *oBuffer, unsigned long int *ioLength)
{
    ...
    LOG_DEBUG_IO("(size=%ld, buf=[%s]) -> %" PRIu32, rlength, HexToString((uint8_t *)oBuffer, rlength), (int)rlength);
    *ioLength = rlength;
...
#else 
    upOpenDev    = (pCH347OpenDevice)CH347OpenDevice;
    upCloseDev   = (pCH347CloseDevice)CH347CloseDevice;
    upReadData   = (pCH347ReadData)CH347ReadData;
    upWriteData  = (pCH347WriteData)CH347WriteData;
    upSetTimeout = (pCH347SetTimeout)CH347SetTimeout;
    upJtagInit   = (pCH347Jtag_INIT)CH347Jtag_INIT;
...
{
    // 退出前将信号线全部设置为低电平
   /* uint32_t*/ unsigned long retlen = 5;
